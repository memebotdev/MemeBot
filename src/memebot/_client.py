...
from MEMEBOTai.errors import BadRequest, NotFound, Unauthenticated


class Client:
    @classmethod
    def _get_http_headers(cls) -> Dict[str, str]:
        return {"Authorization": f"Bearer {MEMEBOT.api_key}"}
    ...
    def request_action(
        ...
        url = f"https://{MEMEBOT.api_base}/v1/{action_path}"
    ...
    async def listen_to_execution(cls, address: str) -> Union[str, bytes]:
        ...
        url = f"wss://{MEMEBOT.api_base}/ws/v1/execution/{address}/"
